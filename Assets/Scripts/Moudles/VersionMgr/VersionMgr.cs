...
public class VersionMgr : Singleton<VersionMgr>
{
    ...
    private string m_versionTxtName = "version.txt";
    ...
    {
        bool isExists = Directory.Exists(MyTool_Unity.DataPath) && File.Exists(MyTool_Unity.DataPath + m_versionTxtName);
        return isExists;
    }
    ...
    {
        string resPath = MyTool_Unity.AppContentPath(); //游戏包资源目录
        string dataPath = MyTool_Unity.DataPath;        //数据目录
    ...
    {
        Debug.Log("开始启动更新流程");
        WWW www = new WWW("file://" + this.LocalPath + m_versionTxtName);
        yield return www;
    ...
    private void LoadRemoteVersionData()
    {
        ...
        _tempTask.Url = this.RemoteUrl + m_versionTxtName;
    ...
    private void AllDownloadTaskFinished(bool updateVersionTxt)
    {
        ...
        if (updateVersionTxt)
        {
            ...
            MyTool.CreateFile(this.LocalPath, m_versionTxtName, this.m_remoteVersionData);
